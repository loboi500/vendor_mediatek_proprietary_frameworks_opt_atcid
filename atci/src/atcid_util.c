...
void ascii_to_hex(char *in_ascii_string, char *out_hex_string, int in_ascii_string_size)
{
    ...
    {
        if (sprintf(hex, "%.2X", in_ascii_string[i]) < 0) {
            break;
        }
...
void  hex_to_ascii(char *in_hex_string, char *out_ascii_string, int in_hex_string_size) {
    int i = 0;
    for (i = 0; i < in_hex_string_size; i++) {
        char c = in_hex_string[i];
        if (c >= '0' && c <= '9') {
            c -= '0';
        } else if (c >= 'A' && c <= 'F') {
            c -= 'A' - 10;
        ...
        if (i % 2 == 0) {
            out_ascii_string[i / 2] = c * 0x10;
        } else {
            out_ascii_string[(i - 1) / 2] += c;
        }
    }

    out_ascii_string[i / 2] = '\0';
}
...
ATOP_t getAtOp(char** cmdLine) {
    ...
    tmpBuf = calloc(1, strlen(*cmdLine) + 1);
    if(tmpBuf == NULL) {
        return AT_NONE_OP;
    }
    strncpy(tmpBuf, *cmdLine, strlen(*cmdLine));

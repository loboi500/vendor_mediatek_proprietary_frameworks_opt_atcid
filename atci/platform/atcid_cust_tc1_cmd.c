...
static ATRESPONSE_t pas_wisetrate(char* rate) {
    ...
    } else if (eWifiTestMode == WIFI_TEST_MODE_80211G_ONLY
            || eWifiTestMode == WIFI_TEST_MODE_80211A_ONLY) {
        int num = 0;
        tmp = rate + 1;
        at_tok_nextint(&tmp, &num);
...
/*    else if(eWifiTestMode == WIFI_TEST_MODE_80211N_ONLY && eWifiBw == WIFI_TEST_BW_40MHZ) {
    ...
    if(eWifiTestMode == WIFI_TEST_MODE_80211N_ONLY &&
            WIFI_TEST_set(RF_AT_FUNCID_PREAMBLE, u4Preamble, NULL, NULL) != 0) {
        return AT_ERROR;
    }
    ...
    /* set GI type */
    if(eWifiTestMode == WIFI_TEST_MODE_80211N_ONLY &&
            WIFI_TEST_set(RF_AT_FUNCID_GI, eGIType, NULL, NULL) != 0) {
        return AT_ERROR;
    }
    gGIType = eGIType;
    ...
    /* set rate code */
    if(WIFI_TEST_set(RF_AT_FUNCID_RATE, u4RateCode, NULL, NULL) == 0) {
        ...
        case AT_SET_OP:
        {
            ...
            if(0 == channel) {
                ...
                if (gWifiTestMode == WIFI_TEST_MODE_80211N_ONLY &&
                        WIFI_TEST_set(RF_AT_FUNCID_PREAMBLE, gPreamble, NULL, NULL) != 0) {
                ...
                /* set GI type */
                if (gWifiTestMode == WIFI_TEST_MODE_80211N_ONLY &&
                        WIFI_TEST_set(RF_AT_FUNCID_GI, gGIType, NULL, NULL) != 0) {
                    LOGATCI(LOG_DEBUG, "pas_witx2_handler RF_AT_FUNCID_GI Failed\n");
                }

                if (WIFI_TEST_set(RF_AT_FUNCID_RATE, gRateCode, NULL, NULL) == 0) {
                    LOGATCI(LOG_DEBUG,
                            "pas_witx2_handler WIFI_TEST_set RF_AT_FUNCID_RATE success.\n");
                } else {
                    LOGATCI(LOG_DEBUG,
                            "pas_witx2_handler WIFI_TEST_set RF_AT_FUNCID_RATE failed \n");
                }
#endif

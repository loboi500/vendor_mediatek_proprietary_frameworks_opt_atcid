...
ATRESPONSE_t pas_csdf_handler(char* cmdline, ATOP_t opType, char* response) {
    ...
    switch(opType) {
        case AT_READ_OP:
            if (sprintf(response, "+CSDF: %d,%d\n", gCsdfMode, gCsdfAuxmode) < 0) {
                LOGATCI(LOG_ERR, "[pas_csdf_handler] sprintf fail");
                return AT_ERROR;
            }
        ...
        case AT_SET_OP:
            ...
            break;
        case AT_TEST_OP:
            if (sprintf(response, "+CSDF: (1,2,3,4,5,6,7),(1,2)\n") < 0) {
                LOGATCI(LOG_ERR, "[pas_csdf_handler] sprintf fail");
                return AT_ERROR;
            }
            LOGATCI(LOG_DEBUG, "[pas_csdf_handler] response %s", response);
    ...
    switch (opType) {
        case AT_READ_OP: {
            ...
            if(time(&timep)) {
                ...
                LOGATCI(LOG_ERR, "get time fail");
                return AT_ERROR;
            }

            if (p == NULL) {
                LOGATCI(LOG_ERR, "p is null from gmtime");
                return AT_ERROR;
            }
            gettimeofday(&tv, &tz);
            ...
            if (gCsdfAuxmode == 1) {
                if (qTz >= 0) {
                    if (sprintf(response, "+CCLK: \"%02d/%02d/%02d,%02d:%02d:%02d+%02d\"\n",
                            (1900+p->tm_year)%100, (1+p->tm_mon), p->tm_mday,
                            p->tm_hour, p->tm_min, p->tm_sec, qTz) < 0) {
                        LOGATCI(LOG_ERR, "[pas_cclk_handler] sprintf fail");
                        return AT_ERROR;
                    }
                } else {
                    if (sprintf(response, "+CCLK: \"%02d/%02d/%02d,%02d:%02d:%02d%03d\"\n",
                            (1900+p->tm_year)%100, (1+p->tm_mon), p->tm_mday,
                            p->tm_hour, p->tm_min, p->tm_sec, qTz) < 0) {
                        LOGATCI(LOG_ERR, "[pas_cclk_handler] sprintf fail");
                        return AT_ERROR;
                ...
                if (qTz >= 0) {
                    if (sprintf(response, "+CCLK: \"%04d/%02d/%02d,%02d:%02d:%02d+%02d\"\n",
                    ...
                    if (sprintf(response, "+CCLK: \"%04d/%02d/%02d,%02d:%02d:%02d%03d\"\n",
                            1900+p->tm_year, (1+p->tm_mon), p->tm_mday,
                            p->tm_hour, p->tm_min, p->tm_sec, qTz) < 0) {
                        LOGATCI(LOG_ERR, "[pas_cclk_handler] sprintf fail");
                        return AT_ERROR;
        ...
        case AT_SET_OP: {
            ...
            *line = '\0';
            ...
            if (strlen(cclkSetCmd) == strlen("yy/MM/dd,hh:mm:ss+tz")) {
                ...
                if (sprintf(response, "CME ERROR: Invalid parameters: [%d] cmdline:%s; [%d] cclkSetCmd:%s",
                        (int) strlen(cmdline), cmdline, (int) strlen(cclkSetCmd), cclkSetCmd) < 0) {
                    LOGATCI(LOG_ERR, "[pas_cclk_handler] sprintf fail");
                }
                return AT_ERROR;
            }
            ...
            if (hasInputTimeZone) {
                if (sscanf(cclkSetCmd, "%2d/%2d/%2d,%2d:%2d:%2d%03d", &setclk.tm_year, &setclk.tm_mon,
                        &setclk.tm_mday, &setclk.tm_hour, &setclk.tm_min, &setclk.tm_sec, &inputTimeZoneValue) < 0) {
                    LOGATCI(LOG_ERR, "[pas_cclk_handler] sscanf fail");
                }
                ...
                if (sscanf(cclkSetCmd, "%2d/%2d/%2d,%2d:%2d:%2d", &setclk.tm_year, &setclk.tm_mon,
                        &setclk.tm_mday, &setclk.tm_hour, &setclk.tm_min, &setclk.tm_sec) < 0) {
                    LOGATCI(LOG_ERR, "[pas_cclk_handler] sscanf fail");
                }
            }
            if (setclk.tm_year < 0 ||
            ...
            if (envTz) {
                ...
                if (mEnvTz) {
                    ...
                    LOGATCI(LOG_ERR, "mEnvTz is null");
                }
            } else {
                unsetenv("TZ");
            }
        ...
        case AT_TEST_OP:
            if (snprintf(response, strlen("+CCLK: yy/MM/dd,hh:mm:ss[+/-tz]") + 1, "%s",
                    "+CCLK: yy/MM/dd,hh:mm:ss[+/-tz]") < 0) {
                LOGATCI(LOG_ERR, "[pas_cclk_handler] snprintf fail");
                return AT_ERROR;
            }
            return AT_OK;
            break;
    ...
    switch (opType) {
        ...
        case AT_READ_OP:
            if (sprintf(response, "%d\r\n", flag) < 0) {
                LOGATCI(LOG_ERR, "[pas_echo_handler] sprintf fail");
                return AT_ERROR;
            }
            return AT_OK;
            break;
        case AT_TEST_OP:
            if (sprintf(response, "0:disable echo, 1:enable echo\r\n") < 0) {
                LOGATCI(LOG_ERR, "[pas_echo_handler] sprintf fail");
                return AT_ERROR;
            }
            return AT_OK;
...
ATRESPONSE_t pas_eurc_handler(char* cmdline, ATOP_t opType, char* response) {
    ...
    switch (opType) {
        ...
        case AT_READ_OP:
            ...
            if (1 == urc_enabled) {
                if (snprintf(response, strlen("+EURC:1\r\n") + 1, "%s", "+EURC:1\r\n") < 0) {
                    LOGATCI(LOG_ERR, "[pas_eurc_handler] snprintf fail");
                    return AT_ERROR;
                }
            } else if (0 == urc_enabled) {
                if (snprintf(response, strlen("+EURC:0\r\n") + 1, "%s", "+EURC:0\r\n") < 0) {
                    LOGATCI(LOG_ERR, "[pas_eurc_handler] snprintf fail");
                    return AT_ERROR;
                }
            } else {
                if (snprintf(response, strlen("+EURC:invalid value.\r\n") + 1, "%s",
                        "+EURC:invalid value.\r\n") < 0) {
                    LOGATCI(LOG_ERR, "[pas_eurc_handler] snprintf fail");
                    return AT_ERROR;
                }
...
ATRESPONSE_t pas_atci_handler(char* cmdline, ATOP_t opType, char* response) {
    int err = 0, cmdID = 0;
    char atci_usermode[MTK_PROPERTY_VALUE_MAX] = {0};
    int usermode = 0;
    ...
    switch (opType) {
        ...
        case AT_READ_OP:
            ...
            if(1 == usermode) {
                if (snprintf(response, strlen("+ATCI:1\r\n") + 1, "%s", "+ATCI:1\r\n") < 0) {
                    LOGATCI(LOG_ERR, "[pas_atci_handler] snprintf fail");
                    return AT_ERROR;
                }
            } else if (0 == usermode) {
                if (snprintf(response, strlen("+ATCI:0\r\n") + 1, "%s", "+ATCI:0\r\n") < 0) {
                    LOGATCI(LOG_ERR, "[pas_atci_handler] snprintf fail");
                    return AT_ERROR;
                }
            } else {
                if (snprintf(response, strlen("+ATCI:invalid value.\r\n") + 1, "%s",
                        "+ATCI:invalid value.\r\n") < 0) {
                    LOGATCI(LOG_ERR, "[pas_atci_handler] snprintf fail");
                    return AT_ERROR;
                }
...
ATRESPONSE_t pas_pct_handler(char* cmdline, ATOP_t opType, char* response) {
    ...
    switch (opType) {
        ...
        case AT_READ_OP:
            ...
            if (1 == pctmode) {
                if (snprintf(response, strlen("+PCT:1\r\n") + 1, "%s", "+PCT:1\r\n") < 0) {
                    LOGATCI(LOG_ERR, "[pas_pct_handler] snprintf fail");
                    return AT_ERROR;
                }
            } else if (0 == pctmode) {
                if (snprintf(response, strlen("+PCT:0\r\n") + 1, "%s", "+PCT:0\r\n") < 0) {
                    LOGATCI(LOG_ERR, "[pas_pct_handler] snprintf fail");
                    return AT_ERROR;
                }
            } else {
                if (snprintf(response, strlen("+PCT:invalid value.\r\n") + 1, "%s",
                        "+PCT:invalid value.\r\n") < 0) {
                    LOGATCI(LOG_ERR, "[pas_pct_handler] snprintf fail");
                    return AT_ERROR;
                }
...
ATRESPONSE_t pas_esuo_handler(char* cmdline, ATOP_t opType, char* response) {
    ...
    switch (opType) {
        ...
        case AT_READ_OP:
            ...
            if (simID == 1) {
                if (snprintf(response, strlen("+ESUO:5\r\n") + 1, "%s", "+ESUO:5\r\n") < 0) {
                    LOGATCI(LOG_ERR, "[pas_esuo_handler] snprintf fail");
                    return AT_ERROR;
                }
            } else if (simID == 2) {
                if (snprintf(response, strlen("+ESUO:6\r\n") + 1, "%s", "+ESUO:6\r\n") < 0) {
                    LOGATCI(LOG_ERR, "[pas_esuo_handler] snprintf fail");
                    return AT_ERROR;
                }
            } else if (simID == 3) {
                if (snprintf(response, strlen("+ESUO:7\r\n") + 1, "%s", "+ESUO:7\r\n") < 0) {
                    LOGATCI(LOG_ERR, "[pas_esuo_handler] snprintf fail");
                    return AT_ERROR;
                }
            } else if (simID == 9) {
                if (snprintf(response, strlen("+ESUO:9\r\n") + 1, "%s", "+ESUO:9\r\n") < 0) {
                    LOGATCI(LOG_ERR, "[pas_esuo_handler] snprintf fail");
                    return AT_ERROR;
                }
            } else if (simID == 0) {
                if (snprintf(response, strlen("+ESUO:4\r\n") + 1, "%s", "+ESUO:4\r\n") < 0) {
                    LOGATCI(LOG_ERR, "[pas_esuo_handler] snprintf fail");
                    return AT_ERROR;
                }
            } else {
                if (snprintf(response, strlen("+ESUO:Value unsupport\r\n") + 1, "%s",
        ...
        case AT_SET_OP: {
            ...
            *line = '\0';
            ...
            if (strlen(config) <= 0) {
                ...
                if (sprintf(response, "\r\n%d\r\nERROR\r\n\r\n", retval) < 0) {
                    LOGATCI(LOG_ERR, "[pas_usbconfig_handler] sprintf fail");
                }
                return AT_ERROR;
            }
...
ATRESPONSE_t pas_vzwatcicfg_handler(char* cmdline, ATOP_t opType, char* response) {
    ...
    switch (opType) {
        ...
        case AT_READ_OP:
            if (VZWAPNE_BYPASS == gVzwapneConfig) {
                if (snprintf(response, strlen("+VZWATCICFG:1\r\n") + 1, "%s", "+VZWATCICFG:1\r\n") < 0) {
                    LOGATCI(LOG_ERR, "[pas_usbconfig_handler] snprintf fail");
                    return AT_ERROR;                }
            } else if (VZWAPNE_SYNC_TO_AP == gVzwapneConfig) {
                if (snprintf(response, strlen("+VZWATCICFG:0\r\n") + 1, "%s", "+VZWATCICFG:0\r\n") < 0) {
                    LOGATCI(LOG_ERR, "[pas_usbconfig_handler] snprintf fail");
                    return AT_ERROR;                }
            } else {
                if (snprintf(response, strlen("+VZWATCICFG:invalid value.\r\n") + 1, "%s",

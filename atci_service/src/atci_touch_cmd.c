            ...
            if (ret == 1) {
                ...
                while (dev_input != NULL) {
                    if (dev_input != NULL && !strncmp(dev_input, "event", strlen("event"))) {
                        if (snprintf(tpd_dev, sizeof(tpd_dev), "%s%s", INPUT_DEVICE_PREFIX, dev_input) < 0) {
                            ALOGE("input dev error");
                            return -1;
                        }
                        fclose(fp);
                        return 0;
...
int touch_down(int x, int y)
{
    char touch_cmd[TOUCH_CMD_LENGTH];

    if (tpd_down == 0) {
        //BTN_TOUCH DOWN
        if (snprintf(touch_cmd, TOUCH_CMD_LENGTH, "sendevent %s 1 330 1", tpd_dev) < 0) {
            ALOGE("touch cmd BTN_TOUCH DOWN error");
            return -1;
        }
        runcmd(touch_cmd);
        tpd_down = 1;
    ...
    //ABS_MT_TOUCH_MAJOR
    if (snprintf(touch_cmd, TOUCH_CMD_LENGTH, "sendevent %s 3 48 40", tpd_dev) < 0) {
        ALOGE("touch cmd ABS_MT_TOUCH_MAJOR error");
        return -1;
    }
    runcmd(touch_cmd);
    //ABS_MT_TRACKING_ID
    if (snprintf(touch_cmd, TOUCH_CMD_LENGTH, "sendevent %s 3 57 0", tpd_dev) < 0) {
        ALOGE("touch cmd ABS_MT_TRACKING_ID error");
        return -1;
    }
    runcmd(touch_cmd);
    //ABS_MT_POSITION_X
    if (snprintf(touch_cmd, TOUCH_CMD_LENGTH, "sendevent %s 3 53 %d", tpd_dev, x) < 0) {
        ALOGE("touch cmd ABS_MT_POSITION_X error");
        return -1;
    }
    runcmd(touch_cmd);
    //ABS_MT_POSITION_Y
    if (snprintf(touch_cmd, TOUCH_CMD_LENGTH, "sendevent %s 3 54 %d", tpd_dev, y) < 0) {
        ALOGE("touch cmd ABS_MT_POSITION_Y error");
        return -1;
    }
    runcmd(touch_cmd);
    //SYN_MT_REPORT
    if (snprintf(touch_cmd, TOUCH_CMD_LENGTH, "sendevent %s 0 2 0", tpd_dev) < 0) {
        ALOGE("touch cmd SYN_MT_REPORT error");
        return -1;
    }
    runcmd(touch_cmd);
    //SYN_REPORT
    if (snprintf(touch_cmd, TOUCH_CMD_LENGTH, "sendevent %s 0 0 0", tpd_dev) < 0) {
        ALOGE("touch cmd SYN_REPORT error");
        return -1;
    }
    runcmd(touch_cmd);
...
int touch_up()
{
    ...
    tpd_down = 0;
    //BTN_TOUCH UP
    if (snprintf(touch_cmd, TOUCH_CMD_LENGTH, "sendevent %s 1 330 0", tpd_dev) < 0) {
        ALOGE("touch cmd BTN_TOUCH UP error");
        return -1;
    }
    runcmd(touch_cmd);
    //SYN_MT_REPORT
...
int touch_tap(int x, int y)
{
    ...
    ret = touch_down(x, y);
    if (ret < 0) {
        ALOGE("touch down error");
        return ret;
    }
    ret = touch_up();
    if (ret < 0) {
        ALOGE("touch up error");
        return ret;
    }
...
int touch_emu_cmd(TP_POINT points[], int point_num)
{
    int i;
    int ret = 0;
    ...
    for (i=0; i<point_num; i++)
    {
        ...
        if (points[i].action == TP_EMU_DEPRESS) //Down
        {
            ...
            ret = touch_down(points[i].x, points[i].y);
            if (ret < 0) {
                ALOGE("Down error");
                break;
            }
        ...
        else if(points[i].action == TP_EMU_SINGLETAP) //Single tap
        {
            ...
            ret = touch_tap(points[i].x, points[i].y);
            break;
        }
...
int touch_cmd_handler(char* cmdline, ATOP_t at_op, char* response)
{
    ...
    switch(at_op){
        ...
        case AT_TEST_OP:
            if (sprintf(response, "\r\n+CTSA: (0,1,2,3)\r\n") < 0) {
                ALOGE("AT_ACTION_OP or AT_READ_OP or AT_TEST_OP resp failed");
                return -1;
            }
            break;
        case AT_SET_OP:
            ...
            {
                if (sprintf(response,"\r\n+CME ERROR: 50\r\n") < 0) {
                    ALOGE("AT_SET_OP resp failed");
                    return -1;
                }
                break;
            }
            if (touch_emu_cmd(points, point_num) < 0) {
                if (sprintf(response,"\r\n+CME ERROR: 50\r\n") < 0) {
                    ALOGE("AT_SET_OP resp failed");
                    return -1;
                }

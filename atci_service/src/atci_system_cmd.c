...
static void* restart_routine(void *arg) {
    ...
    syscall(__NR_reboot, LINUX_REBOOT_MAGIC1, LINUX_REBOOT_MAGIC2, LINUX_REBOOT_CMD_RESTART2, REBSTART_REASON);
    return 0;
}
...
int system_fboot_handler(char* cmdline, ATOP_t at_op, char* response){
    ...
    switch(at_op){
        ...
        case AT_SET_OP:
            ...
            if (sprintf(response, "\r\n%d\r\nOK\r\n\r\n", retval) < 0) {
                ALOGE("faile to sprintf response");
            }
            break;
        default:
...
int system_restart_handler(char* cmdline, ATOP_t at_op, char* response){
    ...
    switch (at_op) {
        ...
        case AT_TEST_OP:
        case AT_SET_OP:
            if (at_op == AT_SET_OP && err) {
                if (sprintf(response, "\r\n\r\n%s\r\n\r\n", RESTART_FAIL) < 0) {
                    ALOGE("fail to sprintf response");
                }
            } else {
                if (invoke_restart() != 0) {
                    if (sprintf(response, "\r\n\r\n%s\r\n\r\n", RESTART_FAIL) < 0) {
...
int system_safe_restart_handler(char* cmdline, ATOP_t at_op, char* response) {
    UNUSED(at_op);
    UNUSED(response);
    ALOGD("[%s] %s", __FUNCTION__, cmdline);
